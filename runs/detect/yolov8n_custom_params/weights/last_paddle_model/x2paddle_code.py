...
class DetectionModel(paddle.nn.Layer):
    def __init__(self):
        ...
        self.conv2d0 = paddle.nn.Conv2D(stride=2, padding=1, out_channels=8, kernel_size=(3, 3), in_channels=3)
        self.silu0 = paddle.nn.Silu()
        self.conv2d1 = paddle.nn.Conv2D(stride=2, padding=1, out_channels=16, kernel_size=(3, 3), in_channels=8)
        self.silu1 = paddle.nn.Silu()
        self.conv2d2 = paddle.nn.Conv2D(out_channels=16, kernel_size=(1, 1), in_channels=16)
        ...
        self.conv2d5 = paddle.nn.Conv2D(out_channels=16, kernel_size=(1, 1), in_channels=24)
        self.silu5 = paddle.nn.Silu()
        self.conv2d6 = paddle.nn.Conv2D(stride=2, padding=1, out_channels=32, kernel_size=(3, 3), in_channels=16)
        self.silu6 = paddle.nn.Silu()
        self.conv2d7 = paddle.nn.Conv2D(out_channels=32, kernel_size=(1, 1), in_channels=32)
        self.silu7 = paddle.nn.Silu()
        self.conv2d8 = paddle.nn.Conv2D(padding=1, out_channels=16, kernel_size=(3, 3), in_channels=16)
        self.silu8 = paddle.nn.Silu()
        self.conv2d9 = paddle.nn.Conv2D(padding=1, out_channels=16, kernel_size=(3, 3), in_channels=16)
        self.silu9 = paddle.nn.Silu()
        self.conv2d10 = paddle.nn.Conv2D(padding=1, out_channels=16, kernel_size=(3, 3), in_channels=16)
        self.silu10 = paddle.nn.Silu()
        self.conv2d11 = paddle.nn.Conv2D(padding=1, out_channels=16, kernel_size=(3, 3), in_channels=16)
        self.silu11 = paddle.nn.Silu()
        self.conv2d12 = paddle.nn.Conv2D(out_channels=32, kernel_size=(1, 1), in_channels=64)
        self.silu12 = paddle.nn.Silu()
        self.conv2d13 = paddle.nn.Conv2D(stride=2, padding=1, out_channels=64, kernel_size=(3, 3), in_channels=32)
        self.silu13 = paddle.nn.Silu()
        self.conv2d14 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=64)
        ...
        self.conv2d18 = paddle.nn.Conv2D(padding=1, out_channels=32, kernel_size=(3, 3), in_channels=32)
        self.silu18 = paddle.nn.Silu()
        self.conv2d19 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=128)
        self.silu19 = paddle.nn.Silu()
        self.conv2d20 = paddle.nn.Conv2D(stride=2, padding=1, out_channels=128, kernel_size=(3, 3), in_channels=64)
        ...
        self.conv2d22 = paddle.nn.Conv2D(padding=1, out_channels=64, kernel_size=(3, 3), in_channels=64)
        self.silu22 = paddle.nn.Silu()
        self.conv2d23 = paddle.nn.Conv2D(padding=1, out_channels=64, kernel_size=(3, 3), in_channels=64)
        self.silu23 = paddle.nn.Silu()
        self.conv2d24 = paddle.nn.Conv2D(out_channels=128, kernel_size=(1, 1), in_channels=192)
        self.silu24 = paddle.nn.Silu()
        self.conv2d25 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=128)
        ...
        self.conv2d26 = paddle.nn.Conv2D(out_channels=128, kernel_size=(1, 1), in_channels=256)
        self.silu26 = paddle.nn.Silu()
        self.conv2d27 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=192)
        self.silu27 = paddle.nn.Silu()
        self.conv2d28 = paddle.nn.Conv2D(padding=1, out_channels=32, kernel_size=(3, 3), in_channels=32)
        self.silu28 = paddle.nn.Silu()
        self.conv2d29 = paddle.nn.Conv2D(padding=1, out_channels=32, kernel_size=(3, 3), in_channels=32)
        self.silu29 = paddle.nn.Silu()
        self.conv2d30 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=96)
        self.silu30 = paddle.nn.Silu()
        self.conv2d31 = paddle.nn.Conv2D(out_channels=32, kernel_size=(1, 1), in_channels=96)
        self.silu31 = paddle.nn.Silu()
        self.conv2d32 = paddle.nn.Conv2D(padding=1, out_channels=16, kernel_size=(3, 3), in_channels=16)
        self.silu32 = paddle.nn.Silu()
        self.conv2d33 = paddle.nn.Conv2D(padding=1, out_channels=16, kernel_size=(3, 3), in_channels=16)
        self.silu33 = paddle.nn.Silu()
        self.conv2d34 = paddle.nn.Conv2D(out_channels=32, kernel_size=(1, 1), in_channels=48)
        self.silu34 = paddle.nn.Silu()
        self.conv2d35 = paddle.nn.Conv2D(stride=2, padding=1, out_channels=32, kernel_size=(3, 3), in_channels=32)
        self.silu35 = paddle.nn.Silu()
        self.conv2d36 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=96)
        ...
        self.conv2d38 = paddle.nn.Conv2D(padding=1, out_channels=32, kernel_size=(3, 3), in_channels=32)
        self.silu38 = paddle.nn.Silu()
        self.conv2d39 = paddle.nn.Conv2D(out_channels=64, kernel_size=(1, 1), in_channels=96)
        self.silu39 = paddle.nn.Silu()
        self.conv2d40 = paddle.nn.Conv2D(stride=2, padding=1, out_channels=64, kernel_size=(3, 3), in_channels=64)
        self.silu40 = paddle.nn.Silu()
        self.conv2d41 = paddle.nn.Conv2D(out_channels=128, kernel_size=(1, 1), in_channels=192)
        ...
        self.conv2d43 = paddle.nn.Conv2D(padding=1, out_channels=64, kernel_size=(3, 3), in_channels=64)
        self.silu43 = paddle.nn.Silu()
        self.conv2d44 = paddle.nn.Conv2D(out_channels=128, kernel_size=(1, 1), in_channels=192)
        ...
        self.conv2d45 = paddle.nn.Conv2D(padding=1, out_channels=64, kernel_size=(3, 3), in_channels=32)
        ...
        self.conv2d48 = paddle.nn.Conv2D(padding=1, out_channels=32, kernel_size=(3, 3), in_channels=32)
        self.silu47 = paddle.nn.Silu()
        self.conv2d49 = paddle.nn.Conv2D(padding=1, out_channels=32, kernel_size=(3, 3), in_channels=32)
        self.silu48 = paddle.nn.Silu()
        self.conv2d50 = paddle.nn.Conv2D(out_channels=2, kernel_size=(1, 1), in_channels=32)
        self.conv2d51 = paddle.nn.Conv2D(padding=1, out_channels=64, kernel_size=(3, 3), in_channels=64)
        ...
        self.conv2d54 = paddle.nn.Conv2D(padding=1, out_channels=32, kernel_size=(3, 3), in_channels=64)
        self.silu51 = paddle.nn.Silu()
        self.conv2d55 = paddle.nn.Conv2D(padding=1, out_channels=32, kernel_size=(3, 3), in_channels=32)
        self.silu52 = paddle.nn.Silu()
        self.conv2d56 = paddle.nn.Conv2D(out_channels=2, kernel_size=(1, 1), in_channels=32)
        ...
        self.conv2d60 = paddle.nn.Conv2D(padding=1, out_channels=32, kernel_size=(3, 3), in_channels=128)
        self.silu55 = paddle.nn.Silu()
        self.conv2d61 = paddle.nn.Conv2D(padding=1, out_channels=32, kernel_size=(3, 3), in_channels=32)
        self.silu56 = paddle.nn.Silu()
        self.conv2d62 = paddle.nn.Conv2D(out_channels=2, kernel_size=(1, 1), in_channels=32)
    ...
    def forward(self, x0):
        ...
        x830 = paddle.reshape(x=x712, shape=[1, 66, -1])
        x832 = paddle.reshape(x=x770, shape=[1, 66, -1])
        x834 = paddle.reshape(x=x828, shape=[1, 66, -1])
        x835 = [x830, x832, x834]
        x836 = paddle.concat(x=x835, axis=2)
        x838 = paddle.split(x=x836, num_or_sections=[64, 2], axis=1)
        x839, x840 = x838
...
def main(x0):
    ...
    params = paddle.load(r'/media/yura/edbf3aee-91a2-4ee8-98ca-fb2ac8d9fc21/processing_gan/runs/detect/yolov8n_custom_params/weights/last_paddle_model/model.pdparams')
